/**
 * Word Document Exporter (V3)
 */
if (typeof Exporter !== 'undefined') {
    Exporter.exportAllCombinedToWordV3 = async function(app) {
        ...
        const btn = document.getElementById('save-all-combined-word-v3');
        ...
        docSections.push(new Paragraph({
            children: [new TextRun({ text: "Comprehensive Pattern Reference", size: 36 })],
            alignment: docx.AlignmentType.CENTER,
            spacing: { after: 400 }
        }));
        ...
        docSections.push(new Paragraph({
            children: [new TextRun({ text: "Table of Contents / Overview", bold: true, size: 36 })],
            spacing: { after: 200 },
            pageBreakBefore: true // Breaks BEFORE TOC
        }));

        for (const key of keysToExport) {
            docSections.push(new Paragraph({
                children: [new TextRun({ text: `Key of ${key}`, bold: true, size: 28 })],
                spacing: { before: 200, after: 100 }
            }));
            for (const cat of categories) {
                docSections.push(new Paragraph({
                    children: [new TextRun({ text: `  ${cat.name}`, bold: true, size: 24 })],
                ...
                for (const [patternId, patternData] of Object.entries(cat.data)) {
                    ...
                    let intervals = [];
                    if (cat.type === 'scale') {
                        notes = app.musicTheory.getScaleNotes(key, patternId);
                        intervals = patternData.intervals || [];
                    } else if (cat.type === 'chord') {
                        notes = app.musicTheory.getChordNotes(key, patternId);
                        intervals = patternData.intervals || [];
                    } else if (cat.type === 'interval') {
                        const targetNote = app.musicTheory.getNoteFromInterval(key, patternId);
                        notes = [key, targetNote];
                        intervals = [patternId];
                    }
                    const patternName = patternData.name || patternId;
                    const line = `    • ${patternName} | Notes: ${notes.join(' - ')} | Intervals: ${intervals.join(', ')}`;
                    docSections.push(new Paragraph({
        ...
        try {
            ...
            for (const cat of categories) {
                ...
                docSections.push(new Paragraph({
                    ...
                    pageBreakBefore: firstContentParagraph
                }));
                ...
                for (const key of keysToExport) {
                    ...
                    for (const [patternId, patternData] of entries) {
                        ...
                        // --- V3 ABOUT PLACEHOLDER ---
                        docSections.push(new Paragraph({
                            children: [
                                new TextRun({ text: `About ${patternName}:`, bold: true, size: 24 })
                            ],
                            spacing: { before: 100, after: 50 }
                        }));
                        
                        let desc = '';
                        if (cat.type === 'scale' && app.musicTheory.getScaleDescription) desc = app.musicTheory.getScaleDescription(patternId);
                        else if (cat.type === 'chord' && app.musicTheory.getChordDescription) desc = app.musicTheory.getChordDescription(patternId);
                        else if (cat.type === 'interval' && app.musicTheory.getIntervalQuality) {
                            const q = app.musicTheory.getIntervalQuality(patternId);
                            desc = `Quality: ${q}. Distance: ${patternData.semitones} semitones.`;
                        }
                        
                        docSections.push(new Paragraph({
                            children: [
                                new TextRun({ text: desc || "[Placeholder for pattern description and additional details]", size: 22 })
                            ],
                            spacing: { after: 300 }
                        }));
            ...
            if (!app.stopExporting) {
                ...
                downloadLink.download = `all_patterns_v3_${originalStringCount}-strings_${dateStr}.docx`;
